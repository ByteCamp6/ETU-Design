...
const useDatePickerStates = (props) => {
  ...
  return vue.reactive({
    ...
    valueFormat: props.valueFormat ? props.valueFormat : utils.initFormate(props.type),
    selected: []
  });
};
const useDatePicker = (states, props, emit) => {
  ...
  const updateModel = (index = -1) => {
    if (index !== -1) {
      ...
      emit("update:modelValue", utils.getFormate(states.value, states.valueFormat));
      if (states.selected.length === 2)
        states.selected = [];
      states.selected.push(states.model[index]);
      if (states.selected.length <= 1) {
        states.startLabel = utils.getFormate(states.model[index], states.format);
      } else {
