...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props) {
    ...
    const { pageCount, disabled, currentPage, changeEvent } =
      usePagination.usePagination();
    const userInput = vue.ref();
    const innerValue = vue.computed(
      () =>
        userInput.value ?? (currentPage == null ? void 0 : currentPage.value),
    );
            ...
            vue.createVNode(
              ...
              ),
              {
                class: vue.normalizeClass([
                  vue.unref(ns).e("editor"),
                  vue.unref(ns).is("in-pagination"),
                ]),
                min: 1,
