...
const useCheckboxDisabled = ({
  ...
  const isLimitDisabled = computed(() => {
    ...
    return !isUndefined(max) && isArray(model.value) && model.value.length >= max && !isChecked.value || !isUndefined(min) && isArray(model.value) && model.value.length <= min && isChecked.value;
  });
