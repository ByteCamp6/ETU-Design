...
const { Checkbox: EtuCheckbox } = checkbox;
const { Radio: EtuRadio } = radio;
const useTableColumn = (props, hasKey, emit) => {
  ...
  const leftFixedColumns = computed(() => {
    ...
    const columns = props.columns.filter(
      (column) => (column == null ? void 0 : column.fixed) === "left",
    );
  ...
  const rightFixedColumns = computed(() => {
    let styleOffsetWidth = 0;
    const columns = props.columns.filter(
      (column) => (column == null ? void 0 : column.fixed) === "right",
    );
    columns.reverse();
    columns.forEach((column) => {
      column.styleOffsetWidth = styleOffsetWidth;
